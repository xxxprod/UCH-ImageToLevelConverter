...
public class Encoder : ICoder, ISetCoderProperties, IWriteCoderProperties
{
	...
	private void Init()
	{
		...
		{
			_posSlotEncoder[num4].Init();
		}
		for (uint num5 = 0u; num5 < 114; num5++)
		{
			_posEncoders[num5].Init();
		}
	...
	private uint GetOptimum(uint position, out uint backRes)
	{
		...
		{
			uint num11 = repLens[num10];
			if (num11 < 2)
			{
				continue;
			}
			uint num12 = num6 + GetPureRepPrice(num10, _state, num5);
			do
			{
				uint num13 = num12 + _repMatchLenEncoder.GetPrice(num11 - 2, num5);
				Optimal optimal = _optimum[num11];
				if (num13 < optimal.Price)
				{
					optimal.Price = num13;
					optimal.PosPrev = 0u;
					optimal.BackPrev = num10;
					optimal.Prev1IsChar = false;
				}
			}
			while (--num11 >= 2);
		}
		uint num14 = price + _isRep[_state.Index].GetPrice0();
		num9 = ((repLens[0] >= 2) ? (repLens[0] + 1) : 2u);
		if (num9 <= lenRes)
		{
			uint num15;
			for (num15 = 0u; num9 > _matchDistances[num15]; num15 += 2)
			{
			}
			while (true)
			{
				uint num16 = _matchDistances[num15 + 1];
				uint num17 = num14 + GetPosLenPrice(num16, num9, num5);
				Optimal optimal2 = _optimum[num9];
				if (num17 < optimal2.Price)
				{
					optimal2.Price = num17;
					optimal2.PosPrev = 0u;
					optimal2.BackPrev = num16 + 4;
					optimal2.Prev1IsChar = false;
				}
				if (num9 == _matchDistances[num15])
				{
					num15 += 2;
					if (num15 == numDistancePairs)
					{
						break;
					}
				}
		...
		while (true)
		{
			...
			position++;
			uint num19 = _optimum[num18].PosPrev;
			Base.State state;
			if (_optimum[num18].Prev1IsChar)
			{
				num19--;
				if (_optimum[num18].Prev2)
				{
					state = _optimum[_optimum[num18].PosPrev2].State;
					if (_optimum[num18].BackPrev2 < 4)
					{
						state.UpdateRep();
					}
					else
			...
			{
				uint num20;
				if (_optimum[num18].Prev1IsChar && _optimum[num18].Prev2)
				{
					num19 = _optimum[num18].PosPrev2;
					num20 = _optimum[num18].BackPrev2;
					state.UpdateRep();
				}
				else
				{
					num20 = _optimum[num18].BackPrev;
					if (num20 < 4)
					{
						state.UpdateRep();
					}
				...
				Optimal optimal3 = _optimum[num19];
				switch (num20)
				{
				case 0u:
			...
			_optimum[num18].State = state;
			_optimum[num18].Backs0 = reps[0];
			_optimum[num18].Backs1 = reps[1];
			_optimum[num18].Backs2 = reps[2];
			_optimum[num18].Backs3 = reps[3];
			uint price2 = _optimum[num18].Price;
			indexByte = _matchFinder.GetIndexByte(-1);
			indexByte2 = _matchFinder.GetIndexByte((int)(0 - reps[0] - 1 - 1));
			num5 = position & _posStateMask;
			uint num21 = price2 + _isMatch[(state.Index << 4) + num5].GetPrice0() + _literalEncoder.GetSubCoder(position, _matchFinder.GetIndexByte(-2)).GetPrice(!state.IsCharState(), indexByte2, indexByte);
			Optimal optimal4 = _optimum[num18 + 1];
			bool flag = false;
			if (num21 < optimal4.Price)
			{
				optimal4.Price = num21;
				optimal4.PosPrev = num18;
				optimal4.MakeAsChar();
				flag = true;
			}
			price = price2 + _isMatch[(state.Index << 4) + num5].GetPrice1();
			num6 = price + _isRep[state.Index].GetPrice1();
			if (indexByte2 == indexByte && (optimal4.PosPrev >= num18 || optimal4.BackPrev != 0))
			{
				uint num22 = num6 + GetRepLen1Price(state, num5);
				if (num22 <= optimal4.Price)
				{
					optimal4.Price = num22;
					optimal4.PosPrev = num18;
			...
			if (!flag && indexByte2 != indexByte)
			{
				...
				if (matchLen >= 2)
				{
					...
					state2.UpdateChar();
					uint num23 = (position + 1) & _posStateMask;
					uint num24 = num21 + _isMatch[(state2.Index << 4) + num23].GetPrice1() + _isRep[state2.Index].GetPrice1();
					uint num25 = num18 + 1 + matchLen;
					while (num8 < num25)
					{
						_optimum[++num8].Price = 268435455u;
					}
					uint num26 = num24 + GetRepPrice(0u, matchLen, state2, num23);
					Optimal optimal5 = _optimum[num25];
					if (num26 < optimal5.Price)
					{
						optimal5.Price = num26;
						optimal5.PosPrev = num18 + 1;
			...
			{
				uint num29 = _matchFinder.GetMatchLen(-1, reps[num28], num);
				if (num29 < 2)
				{
					continue;
				}
				...
				while (true)
				{
					...
					uint num31 = num6 + GetRepPrice(num28, num29, state, num5);
					Optimal optimal6 = _optimum[num18 + num29];
					if (num31 < optimal6.Price)
					{
						optimal6.Price = num31;
						optimal6.PosPrev = num18;
						optimal6.BackPrev = num28;
				...
				uint limit2 = Math.Min(val - 1 - num29, _numFastBytes);
				uint matchLen2 = _matchFinder.GetMatchLen((int)num29, reps[num28], limit2);
				if (matchLen2 >= 2)
				{
					Base.State state3 = state;
					state3.UpdateRep();
					uint num32 = (position + num29) & _posStateMask;
					uint num33 = num6 + GetRepPrice(num28, num29, state, num5) + _isMatch[(state3.Index << 4) + num32].GetPrice0() + _literalEncoder.GetSubCoder(position + num29, _matchFinder.GetIndexByte((int)(num29 - 1 - 1))).GetPrice(matchMode: true, _matchFinder.GetIndexByte((int)(num29 - 1 - (reps[num28] + 1))), _matchFinder.GetIndexByte((int)(num29 - 1)));
					state3.UpdateChar();
					num32 = (position + num29 + 1) & _posStateMask;
					uint num34 = num33 + _isMatch[(state3.Index << 4) + num32].GetPrice1() + _isRep[state3.Index].GetPrice1();
					uint num35 = num29 + 1 + matchLen2;
					while (num8 < num18 + num35)
					{
						_optimum[++num8].Price = 268435455u;
					}
					uint num36 = num34 + GetRepPrice(0u, matchLen2, state3, num32);
					Optimal optimal7 = _optimum[num18 + num35];
					if (num36 < optimal7.Price)
					{
						optimal7.Price = num36;
						optimal7.PosPrev = num18 + num29 + 1;
						optimal7.BackPrev = 0u;
						optimal7.Prev1IsChar = true;
						optimal7.Prev2 = true;
						optimal7.PosPrev2 = num18;
						optimal7.BackPrev2 = num28;
					}
				}
			}
			if (lenRes2 > num)
			...
			num14 = price + _isRep[state.Index].GetPrice0();
			while (num8 < num18 + lenRes2)
			{
				_optimum[++num8].Price = 268435455u;
			}
			uint num37;
			for (num37 = 0u; num27 > _matchDistances[num37]; num37 += 2)
			{
			}
			uint num38 = num27;
			while (true)
			{
				...
				uint num40 = num14 + GetPosLenPrice(num39, num38, num5);
				Optimal optimal8 = _optimum[num18 + num38];
				if (num40 < optimal8.Price)
				{
					optimal8.Price = num40;
					optimal8.PosPrev = num18;
				...
				if (num38 == _matchDistances[num37])
				{
					if (num38 < val)
					{
						...
						if (matchLen3 >= 2)
						{
							...
							uint num43 = num42 + _isMatch[(state4.Index << 4) + num41].GetPrice1() + _isRep[state4.Index].GetPrice1();
							uint num44 = num38 + 1 + matchLen3;
							while (num8 < num18 + num44)
							{
								_optimum[++num8].Price = 268435455u;
							}
							num40 = num43 + GetRepPrice(0u, matchLen3, state4, num41);
							optimal8 = _optimum[num18 + num44];
							if (num40 < optimal8.Price)
							{
								optimal8.Price = num40;
								optimal8.PosPrev = num18 + num38 + 1;
								optimal8.BackPrev = 0u;
								optimal8.Prev1IsChar = true;
								optimal8.Prev2 = true;
								optimal8.PosPrev2 = num18;
	...
	private void FillDistancesPrices()
	{
		...
		for (uint num4 = 0u; num4 < 4; num4++)
		{
			...
			for (uint num7 = 14u; num7 < _distTableSize; num7++)
			{
				_posSlotPrices[num5 + num7] += (num7 >> 1) - 1 - 4 << 6;
			}
			uint num8 = num4 * 128;
			uint num9;
			for (num9 = 0u; num9 < 4; num9++)
			{
				_distancesPrices[num8 + num9] = _posSlotPrices[num5 + num9];
			}
			for (; num9 < 128; num9++)
			{
				_distancesPrices[num8 + num9] = _posSlotPrices[num5 + GetPosSlot(num9)] + tempPrices[num9];
			}
		}
		_matchPriceCount = 0u;
	}
	...
	public void SetCoderProperties(CoderPropID[] propIDs, object[] properties)
	{
		for (uint num = 0u; num < properties.Length; num++)
		{
			...
			case CoderPropID.MatchFinder:
			{
				...
				int num7 = FindMatchFinder(((string)obj).ToUpper());
				if (num7 < 0)
				{
					throw new InvalidParamException();
				}
				_matchFinderType = (EMatchFinderType)num7;
			...
			{
				if (!(obj is int num6) || 1 == 0)
				{
					throw new InvalidParamException();
				}
				if ((long)num6 < 1L || (long)num6 > 1073741824L)
				{
					throw new InvalidParamException();
				}
				_dictionarySize = (uint)num6;
				int i;
				for (i = 0; (long)i < 30L && num6 > (uint)(1 << i); i++)
				{
				}
